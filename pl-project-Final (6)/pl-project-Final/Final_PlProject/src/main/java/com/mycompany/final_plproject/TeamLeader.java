...
public void addTeamMember(String fName, String lName, int id) {
        
        try {
            if (!empFile.exists()) empFile.createNewFile();
            if (!Teamfile.exists()) Teamfile.createNewFile();
        } catch (IOException e) {
            JOptionPane.showMessageDialog(null, "Error with files: " + e.getMessage());
        }
...
public void deleteTeamMember(String fName, String lName, int id) {
    ...
    try (Scanner reader = new Scanner(Teamfile)) {
        ...
        javax.swing.JOptionPane.showMessageDialog(null, "Team file not found.");
        return;
    }
    ...
    for (String line : lines) {
        ...
        try {
            fileID = Integer.parseInt(parts[1].trim());
    ...
    if (!found) {
        javax.swing.JOptionPane.showMessageDialog(null, "Error: Employee does NOT exist!");
        return;
    }
    ...
    try (PrintWriter writer = new PrintWriter(Teamfile)) {
        ...
        javax.swing.JOptionPane.showMessageDialog(null, "Error writing to file.");
    }
}
